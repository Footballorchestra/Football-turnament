...
/* --- Автообновление данных у зрителей, миллисекунды (0 — выключено) ---
   ...
   посетителю не нужно ничего нажимать. Дополнительно данные обновляются сразу,
...
window.FT_CONFIG.photo = Object.assign({
    ...
    maxSize: 512,                      // сторона квадрата, px (фото игроков и эмблемы)
    ...
    // Фотографии команды (галерея на странице команды): пропорции сохраняются,
    // длинная сторона — до galleryMaxSize, поэтому при открытии на весь экран
    // картинка остаётся чёткой. Каждое фото — отдельный файл в репозитории.
    galleryMaxSize: 1920,              // длинная сторона фото команды, px
    galleryQuality: 0.86,              // качество JPEG для фото команды
    galleryMaxResultBytes: 1200 * 1024, // после сжатия: до 1,2 МБ
